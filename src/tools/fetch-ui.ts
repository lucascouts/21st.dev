...
const logger = new Logger("FetchUI");
const FETCH_UI_TOOL_NAME = "magic_component_inspiration";
...
export class FetchUiTool extends BaseTool {
  ...
  async execute({ message, searchQuery }: z.infer<typeof this.schema>) {
    try {
      logger.info(`Fetching UI inspiration...`);
      logger.debug(`Search: ${searchQuery}`);

      const { data, status } = await twentyFirstClient.post<FetchUiResponse>(
        "/api/fetch-ui",
        {
          message,
          searchQuery,
      ...
      logger.info(`Successfully fetched inspiration`);
